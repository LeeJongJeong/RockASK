...
import { type DashboardPageSearchParams, loadChatDetailPageData } from "@/lib/route-page-data";
...
export default async function ChatDetailPage({ params, searchParams }: ChatDetailPageProps) {
  ...
  const { detail } = await loadChatDetailPageData(chatId, searchParams);
  ...
  if (!detail) {
    return (
      <AppScreenShell
        eyebrow="채팅 상세"
        title="찾는 채팅이 없습니다"
        description="최근 채팅 목록에 없거나 현재 권한에서 볼 수 없는 대화입니다."
        breadcrumbs={[
      ...
      >
        <AppEmptyState
          title="이 채팅을 찾을 수 없습니다"
          description="권한 범위를 다시 확인하거나 최근 채팅 목록에서 다시 진입해 주세요."
  ...
  const sidebar = (
    <>
      ...
      <AppSectionCard
        title="다음에 바로 물어볼 질문"
        description="같은 맥락으로 이어가기 좋은 후속 질문입니다."
      >
        {detail.followUps.length === 0 ? (
          <p className="text-sm leading-6 text-slate-500">추천 후속 질문이 아직 없습니다.</p>
        ) : (
          <div className="space-y-3">
  ...
  return (
    <AppScreenShell
      ...
      description="검색 결과가 어떤 대화 흐름으로 이어졌는지, 어떤 문서를 근거로 답했는지 이 화면에서 다시 확인할 수 있습니다."
      breadcrumbs={[
        { href: appRoutes.home, label: "홈" },
        { href: appRoutes.chats, label: "채팅" },
    ...
    >
      ...
      <AppSectionCard
        title="답변 요약"
        description="이 대화에서 전달된 핵심 답변만 먼저 보여 줍니다."
      >
      ...
      <AppSectionCard
        title="대화 흐름"
        description="사용자 질문과 답변의 시간 순서를 그대로 보여 줍니다."
      >
