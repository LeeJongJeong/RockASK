import type { KnowledgeSpaceSummary } from "@rockask/types";
import Link from "next/link";
import { SectionEmptyState } from "@/components/landing/section-empty-state";
import { formatKnowledgeSpaceStatusLabel } from "@/lib/dashboard-formatters";
import { appRoutes } from "@/lib/routes";

const knowledgeSpaceStyles = {
...
export function KnowledgeSpacesSection({ spaces }: { spaces: KnowledgeSpaceSummary[] }) {
  return (
      ...
      <div className="mt-5 space-y-4">
        {spaces.length === 0 ? (
          <SectionEmptyState
            ...
            description="컬렉션이 준비되면 최근 접근도가 높은 지식 공간이 이 영역에 나타납니다."
          ...
          spaces.map((space) => (
            <Link
              ...
              className="block rounded-3xl border border-slate-200 p-5 transition hover:border-blue-300 hover:shadow-sm"
            >
              <div className="flex items-start justify-between gap-4">
                ...
                >
                  {formatKnowledgeSpaceStatusLabel(space.status)}
                </span>
              </div>
