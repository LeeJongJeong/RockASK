...
const navItems: SidebarNavItem[] = [
  { label: "대시보드", href: appRoutes.home, active: true },
        ...
        <div className="rounded-3xl bg-slate-950 p-5 text-slate-50 shadow-[0_20px_50px_rgba(15,23,42,0.18)]">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="rounded-full bg-emerald-500/15 px-2 py-1 text-[11px] font-semibold text-emerald-300">
              {alert ? formatAlertSeverityLabel(alert.severity) : "안내"}
            </span>
          </div>
          <div className="mt-4 rounded-2xl bg-white/10 px-3 py-2 text-xs text-slate-200">
            {alert?.title ?? "동기화와 접근 제어 상태를 이 영역에서 빠르게 확인할 수 있습니다."}
          </div>
        </div>
      </div>
    </aside>
